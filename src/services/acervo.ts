...
export async function fetchAcervo(): Promise<WPAcervo[]> {
  return wpFetchWithFallback<WPAcervo[]>(`${BASE}?${WP_LIST_PARAMS}`, acervoMock)
}
...
export async function fetchAcervoById(id: number): Promise<WPAcervo> {
  ...
  try {
    return await wpFetch<WPAcervo>(`${BASE}/${id}?_embed`)
  } catch (err) {
    console.warn(`[WP] Falha ao buscar acervo id=${id}, usando mock:`, err)
    const found = acervoMock.find((item) => item.id === id)
    if (!found) throw err
    return found
  }
...
export async function fetchAcervoBySlug(slug: string): Promise<WPAcervo | null> {
  if (!isWpConfigured()) return acervoMock.find((item) => item.slug === slug) ?? null
  try {
    const results = await wpFetch<WPAcervo[]>(`${BASE}?slug=${slug}&_embed`)
    return results[0] ?? null
  } catch (err) {
    console.warn(`[WP] Falha ao buscar acervo slug=${slug}, usando mock:`, err)
    return acervoMock.find((item) => item.slug === slug) ?? null
  }
}
